...
| 2026-04-14 | LC 2463 (線性指派 DP) | 設計 DP 之前沒先萃取最佳解結構性質，直接卡在 greedy/capacity 邊界。看到「兩組東西配對」第一反應是 greedy 而非先問「會不會交叉」 | 流程：(1) sort 兩邊 (2) 用 exchange argument 證明最佳解 non-crossing (3) state = 前 i 個 A 配前 j 個 B (4) 若有 capacity，transition 枚舉「B_j 吃幾個 A」。關鍵招牌：線性配對 → non-crossing → prefix-vs-prefix DP | 需複習 | 2026-04-17 口頭驗收未通過：transition/邊界對但**識別信號靠題目記憶+直覺**，無法抽離 pattern。已升級 cheatsheet 識別段落（加排除對照 + 救援訊號）。下一步：LC 1478 獨立驗證是否內化 |
| 2026-04-14 | DP transition 窮盡決策 | state 對了但 transition 只寫 `dp[i-1][j]` 和 `dp[i][j-1]`，隱含假設 factory 最多吃 1 個 robot — 漏掉了「一個位置可以吃多個」的決策維度 | 警訊：題目明示 capacity/容量 > 1，或一個 slot 可以服務多個 item，transition 就要**枚舉 k**：`dp[i][j] = min over k of dp[i-k][j-1] + cost(last k items -> slot j)`。和多重背包、partition DP 同一個模式 | 需複習 | 回家寫完 LC 2463 驗證能獨立寫出含 k 的 transition |
...
| 2026-04-15 | Union Find union 寫錯 | `union(u,v)` 直接用 `parent[u]` 當 root，但那只是直接 parent。多層結構時會把非 root 接到別的地方，破壞 union | `union` 永遠用 `find(u)` 和 `find(v)` 拿 root 再 merge。模板：`pu, pv = self.find(u), self.find(v); if pu != pv: parent[pu] = pv` | 需複習 | 下次出 union find 題前口頭考「union 的第一步是什麼」 |