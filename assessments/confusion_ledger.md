...
| 2026-04-12 | LC 1235 Job Scheduling | DP 的 index 第一反應用 time 而非 job。時間範圍大（10^9）時要想到用 index 而非 value 當 DP 維度 | 排序後以 job index 當 dp[i]。下一步：在已排序的 end times 中用 binary search 找不衝突的 job。複雜度 O(n log n) | 已修正 | 2026-04-22 口頭驗收通過：能完整口述 state、transition、binary search 用途、為什麼不用 time 當 index |
...
| 2026-04-14 | LC 2463 (線性指派 DP) | 設計 DP 之前沒先萃取最佳解結構性質，直接卡在 greedy/capacity 邊界。看到「兩組東西配對」第一反應是 greedy 而非先問「會不會交叉」 | 流程：(1) sort 兩邊 (2) 用 exchange argument 證明最佳解 non-crossing (3) state = 前 i 個 A 配前 j 個 B (4) 若有 capacity，transition 枚舉「B_j 吃幾個 A」。關鍵招牌：線性配對 → non-crossing → prefix-vs-prefix DP | 已修正 | 2026-04-22 LC 1478 獨立完成：觀念全自己想出（non-crossing、median、O(1)遞推預處理、DP state+transition），bug 僅限 base case 初始化 |
| 2026-04-14 | DP transition 窮盡決策 | state 對了但 transition 只寫 `dp[i-1][j]` 和 `dp[i][j-1]`，隱含假設 factory 最多吃 1 個 robot — 漏掉了「一個位置可以吃多個」的決策維度 | 警訊：題目明示 capacity/容量 > 1，或一個 slot 可以服務多個 item，transition 就要**枚舉 k**：`dp[i][j] = min over k of dp[i-k][j-1] + cost(last k items -> slot j)`。和多重背包、partition DP 同一個模式 | 已修正 | 2026-04-22 LC 1478 獨立寫出枚舉 h 的 transition |
| 2026-04-14 | LC 2463 transition 漏 case（k=0） | 枚舉 k 從 1 開始，漏掉「factory j 不收任何 robot」的選項，導致非用不可的最佳解被 inf 蓋住 | 含 capacity 的線性指派 DP：k loop 之前先寫 `dp[i][j] = dp[i][j-1]`（factory j 跳過）。或 k loop 從 0 開始且 k=0 時 cost=0、不動 i。**通則：每次寫 transition 先列「不選/不用」這個 trivial option** | 需複習 | 下次出含 capacity 的 DP 題前先口頭問「不收任何個的 case 怎麼處理」 |
| 2026-04-14 | DP loop 邊界靠 Python 負索引「巧合」過 | 寫 `if i - k < 0: break` 但實際存取 `robot[i-k-1]`，邊界差 1。Python 負 index 不報錯 + dp[m][...] 還沒算到剛好是 inf，靠運氣過 | 邊界檢查要對應**實際存取的 index**，不是迴圈變數本身。`access robot[i-k-1]` → `if i-k-1 < 0: break` 或寫成 `range(min(fc, i))`。**規則：寫負 index 邊界時，把實際存取的最小 index 拉出來檢查** | 需複習 | 下次寫 DP 邊界時要求口述「最深處存取的 index 是什麼、什麼條件下會越界」 |
| 2026-04-15 | Union Find union 寫錯 | `union(u,v)` 直接用 `parent[u]` 當 root，但那只是直接 parent。多層結構時會把非 root 接到別的地方，破壞 union | `union` 永遠用 `find(u)` 和 `find(v)` 拿 root 再 merge。模板：`pu, pv = self.find(u), self.find(v); if pu != pv: parent[pu] = pv` | 已修正 | 2026-04-22 口頭驗收通過 |
| 2026-04-22 | DP base case 初始化（LC 1478） | `dp[0][0]`（0 個 item、0 個 slot）漏設成 0，保留 inf。導致所有 `dp[h][1]` 查 `dp[0][0]` 時拿到 inf，j=1 整排壞掉，連帶後續全錯 | **寫 DP 時先把所有 base case 一次寫完再寫 loop。** `dp[0][0]` 是最容易漏的 — 語意是「空集合、空資源 = 成本 0」。規則：初始化後掃一眼 `dp[0][0]`, `dp[0][j]`, `dp[i][0]` 三類確認語意正確 | 需複習 | 下次寫 2D DP 前先口述三類 base case |
| 2026-04-23 | Monotonic Stack 過度設計（LC 1944） | Stack 裡塞 (index, count) tuple 追蹤累計可見人數，試圖傳遞 count 給下一個人。但可見性不能傳遞，導致答案多算 | **Stack 先只存 index，用 ans array 當累加器。** 每次互動（pop 或 push 時 stack top 看到新元素）就 `ans[idx] += 1`。不需要在 stack 裡追蹤額外資訊。**原則：先用最簡單結構，不夠再加** | 需複習 | 下次遇到 monotonic stack 題先問「stack 只存 index 夠不夠」 |
| 2026-04-28 | Monotonic Deque 雙端操作（LC 862） | 想把右端 pop 的條件從「維護單調」改成「找答案」，會 pop 掉未來可能有用的元素。沒意識到「維護結構」和「找答案」是兩個獨立操作 | **右端 pop = 維護單調性**（跟平常一樣）。**左端 pop = 找答案**（用過的 index 未來只會更差，可以安全丟掉）。兩個操作獨立，不能合併成一個。進階：如果不能左端 pop，可在 monotonic 結構上 binary search (O(n log n)) | 已修正 | 2026-04-28 LC 862 獨立完成，deque 雙端操作正確 |